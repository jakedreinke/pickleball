...
function draftInvite(game, friend, host) {
  return `Invited ${friend.name} — ${hostLabel(host)} is hosting pickleball on ${fmtFull(game.date)} at ${game.time}, at ${game.location}. They'll get a notification and can tap "Yes, I'm in" right in the app.`;
}
...
export default function App() {
  ...
  const requestPush = async (id) => {
    const res = await enablePush(id);
    setPushPerm(pushPermission());
    if (res === "granted") showToast("Notifications are on 🔔");
    else if (res === "denied") showToast("Allow notifications in your phone settings to get invite pings");
    return res;
  };
  // Picking your name = a tap, which is exactly what the phone requires to be
  // allowed to ask. So we request notification permission right then, automatically.
  const chooseMe = (id) => {
    setMe(id);
    saveMe(id);
    if (id) requestPush(id);
  };
  const enableNotifications = async () => {
    if (!me) return;
    const res = await requestPush(me);
    if (res === "unsupported") showToast("Add the app to your home screen first, then open it from there");
    else if (res === "error") showToast("Couldn't turn on notifications — try again");
  };
  ...
  const scheduleGame = ({ date, time, hostId, location, invitedIds }) => {
    ...
    invitedIds.forEach((fid) => {
      ...
      if (f) {
        ...
        const emailText = `Hi ${f.name}! ${hostLabel(host)} invited you to pickleball on ${fmtFull(g.date)} at ${g.time}, at ${g.location}. Tap to say yes: ${link}`;
        notifyFriend(fid, "🏓 You're invited to pickleball", `${hostLabel(host)} — ${fmtFull(g.date)} at ${g.time}. Open the app to say yes.`, f.email, emailText);
      }
    });
  ...
  const respond = (gameId, friendId, answer, viaApp = false) => {
    ...
    // Ping the host so they find out about the reply without checking the app.
    if (viaApp && game.hostId && game.hostId !== friendId) {
      notifyFriend(
        game.hostId,
        answer === "yes" ? `✅ ${f.name} is in!` : `${f.name} can't make it`,
        `${fmtShort(game.date)} at ${game.time}`
      );
    }
  };
  ...
  const inviteMore = (gameId, ids) => {
    ...
    ids.forEach((id) => {
      ...
      if (f) {
        ...
        const link = typeof window !== "undefined" ? window.location.origin : "";
        const emailText = `Hi ${f.name}! ${hostLabel(host)} invited you to pickleball on ${fmtFull(game.date)} at ${game.time}, at ${game.location}. Tap to say yes: ${link}`;
        notifyFriend(id, "🏓 You're invited to pickleball", `${hostLabel(host)} — ${fmtFull(game.date)} at ${game.time}. Open the app to say yes.`, f.email, emailText);
      }
    });
        ...
        <main className="kpa-main">
          {detailGame ? (
            <GameDetail
              ...
              onCopyInvite={() => {
                const host = byId[detailGame.hostId];
                const link = typeof window !== "undefined" ? window.location.origin : "";
                const text = `🏓 You're invited to pickleball — ${fmtFull(detailGame.date)} at ${detailGame.time}, ${detailGame.location}${host ? ` (${hostLabel(host)} hosting)` : ""}. Tap to say yes: ${link}`;
...
/* ------------------------------ Game detail ------------------------------ */
function GameDetail({ game, byId, owner, me, onRespond, onInviteMore, onSignup, onCopyInvite, onDelete }) {
  const players = confirmedPlayers(game, byId);
  const host = byId[game.hostId];
  const left = CAP - players.length;
  const full = left <= 0;
  ...
  return (
    <div className="pad">
      ...
      <button className="btn btn-ghost btn-block" style={{ marginTop: 8 }} onClick={onCopyInvite}>
        <Copy size={16} /> Copy invite text (to send the link)
      </button>
...
function FriendSheet({ onClose, onSubmit }) {
  ...
  return (
    <Sheet
      title="Add a friend"
      subtitle="Save their name. Add an email and they'll get invites even without the app."
      onClose={onClose}
      footer={
        <button className="btn btn-primary btn-block btn-lg" disabled={!name.trim()} onClick={() => onSubmit({ name: name.trim(), phone: phone.trim(), email: email.trim(), regular, canHost })}>
    ...
    >
      ...
      <label className="field">
        <span>Email <span style={{ fontWeight: 400, color: "var(--ink-soft)" }}>(gets invites without the app)</span></span>
        <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="linda@email.com" inputMode="email" />
      </label>
      <label className="field">
        <span>Cell number <span style={{ fontWeight: 400, color: "var(--ink-soft)" }}>(optional)</span></span>
